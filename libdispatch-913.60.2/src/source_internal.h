...
struct dispatch_source_s_xx_comment {
	struct dispatch_queue_s_xx_comment *queue;		//source执行的队列相关信息
...
//	dm_cancel_handler_called:1,
//	dm_is_xpc:1;
	uint64_t ds_data DISPATCH_ATOMIC64_ALIGN;   //source中需要处理的数据
	uint64_t ds_pending_data DISPATCH_ATOMIC64_ALIGN;  //source中阻塞数据
};
