...
//						 );
struct dispatch_object_s_xx_comment {
	const struct object_vtable_s *do_vtable;  //函数调用表 push,pop，do_invoke, suspend, resume等操作
	uint32 do_ref_cnt;  //引用计数
	uint32 do_xref_cnt;
	struct _os_object_s _as_os_obj[0];
	struct dispatch_object_s *volatile do_next;  //下一个执行元素
	struct dispatch_queue_s *do_targetq;   //目标队列，object的执行队列
	void *do_ctxt;		 //上下文参数
	void *do_finalizer;  //finally call
};

struct dispatch_queue_s_xx_comment {
	struct os_mpsc_queue_s _as_oq[0];
	DISPATCH_OBJECT_HEADER(x);
	struct dispatch_object_s *volatile dq_items_head;  //任务队列头部 链表形式组织
	DISPATCH_UNION_LE(uint64_t volatile dq_state,
					  dispatch_lock _dq_state_lock,
					  uint32_t dq_state_bits
					  ) DISPATCH_ATOMIC64_ALIGN;  //64位数的dq_state 高32位为锁，低32位为状态位
	unsigned long dq_serialnum;   //队列序列
	const char *dq_label;
	struct dispatch_object_s *volatile dq_items_tail;    //任务队尾 链表形式组织
	dispatch_priority_t dq_priority;	//优先级
	int volatile dq_sref_cnt;
	uint32_t dq_side_suspend_cnt;
	dispatch_unfair_lock_s dq_sidelock;  //队列的自旋休眠锁
	union {  //队列中处理的资源类型  source，timer observer
		dispatch_queue_t dq_specific_q;
		struct dispatch_source_refs_s *ds_refs;
		struct dispatch_timer_source_refs_s *ds_timer_refs;
		struct dispatch_mach_recv_refs_s *dm_recv_refs;
		DISPATCH_UNION_LE(uint32_t volatile dq_atomic_flags,  //原子标记
						  const uint16_t dq_width,   //最大并发数量 1为串行队列 大于1是concurrent
						  const uint16_t __dq_opaque
						  );
	};
};
...
struct dispatch_continuation_s_xx_comment {
	struct dispatch_object_s _as_do[0];
	union {
		const void *do_vtable;
		uintptr_t dc_flags;
	...
	struct dispatch_continuation_s *volatile do_next;  //下一个续体信息
	struct voucher_s *dc_voucher;  //凭证
	dispatch_function_t dc_func;   // dc封装的函数调用
	void *dc_ctxt;  //上下文函数参数
	void *dc_data;  //数据
	void *dc_other;  // dc封装的其他信息
};
