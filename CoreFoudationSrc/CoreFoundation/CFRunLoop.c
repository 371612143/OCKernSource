...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    //使用gcdtimer 创建定时器，当runloop运行了seconds秒超时后调用__CFRunLoopTimeout，__CFRunLoopTimeoutCancel
	...
	__CFRunLoopDoBlocks(rl, rlm);
        ...
        //之前没有处理过source0，也没有source1消息，且没有设置超时，就让线程进入睡眠。
	...
	if (!poll && (rlm->_observerMask & kCFRunLoopAfterWaiting)) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopAfterWaiting);
        ...
        if (MACH_PORT_NULL == livePort) {
            ...
            // 9.3处理Source1 (基于port)// Despite the name, this works for windows handles as well
    ...
    // 进入loop时参数说处理完事件就返回，且本次runloop处理了source
...
//runloop model seconds 运行时长， returnAfterSourceHandled 是否处理完source就会返回
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    CHECK_FOR_FORK();
    if (__CFRunLoopIsDeallocating(rl)) return kCFRunLoopRunFinished;
    __CFRunLoopLock(rl);
    CFRunLoopModeRef currentMode = __CFRunLoopFindMode(rl, modeName, false);
    //没有timeers， source0 source1 马上返回
